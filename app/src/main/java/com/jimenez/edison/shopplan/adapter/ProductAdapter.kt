...
class ProductAdapter(
    ...
    class ProductViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        val weeklyText: TextView = itemView.findViewById(R.id.product_weekly)
    ...
    override fun onBindViewHolder(holder: ProductViewHolder, position: Int) {
        ...
        holder.weeklyText.isVisible = product.addWeekly
        ...
        holder.priceText.text = String.format("$%.2f", product.estimatedPrice)
        ...
        holder.editButton.setOnClickListener { onEditProduct(product) }
        holder.deleteButton.setOnClickListener { onDeleteProduct(product) }
    }
    ...
    fun getTotalPrice(): Double {
        return products
            .filter { selectedProducts.contains(it.id) }
            .sumOf { it.estimatedPrice * it.quantity } // Cálculo corregido: precio * cantidad
    }
}